...
Shader "DawnShaderHDRP/EyesTearHDRP"
{
	Properties
	{
		...
		_Color("Color", Color) = (0.09433961,0.09433961,0.09433961,0)
		_OpacityBoost("Opacity Boost", Range( 0 , 1)) = 0.02
		[ASEEnd]_Distortion("Distortion", Range( 0 , 1)) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float4 fetchOpaqueVal9 = ASEHDSampleSceneColor(( ase_screenPosNorm + float4( ( _Distortion * tex2DNode1 ) , 0.0 ) ).xy, 0.0, GetInverseCurrentExposureMultiplier());
				
				surfaceDescription.Albedo = ( _Color * fetchOpaqueVal9 ).rgb;
				surfaceDescription.Normal = tex2DNode1;
				...
				surfaceDescription.Smoothness = _Roughness;
				surfaceDescription.Occlusion = 1;
				surfaceDescription.Alpha = _OpacityBoost;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float2 uv_NormalMap = packedInput.ase_texcoord1.xy * _NormalMap_ST.xy + _NormalMap_ST.zw;
				float3 tex2DNode1 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap ), 1.0f );
				float4 fetchOpaqueVal9 = ASEHDSampleSceneColor(( ase_screenPosNorm + float4( ( _Distortion * tex2DNode1 ) , 0.0 ) ).xy, 0.0, GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
						...
						)
			{
				...
				float4 ase_screenPosNorm = screenPos / screenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float2 uv_NormalMap = packedInput.ase_texcoord8.xy * _NormalMap_ST.xy + _NormalMap_ST.zw;
				float3 tex2DNode1 = UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap ), 1.0f );
				float4 fetchOpaqueVal9 = ASEHDSampleSceneColor(( ase_screenPosNorm + float4( ( _Distortion * tex2DNode1 ) , 0.0 ) ).xy, 0.0, GetInverseCurrentExposureMultiplier());
...
WireConnection;27;7;12;0
WireConnection;27;9;25;0
ASEEND*/
//CHKSM=2BA2205C611156D184853AF5A7BCAD7BC9B40B64